...
const navItems = [
  ...
  { label: 'How to Join', section: 'how-to-join' },
...
export const NavigationMenu = ({
  ...
  const playersText = isLoading ? '—/—' : `${playerCount}/${maxPlayers}`;
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 z-40 backdrop-blur-sm sm:backdrop-blur-xl bg-black/40 sm:bg-white/5"
            onClick={onClose}
          />
          ...
          >
            ...
            <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-full glass-button z-10 sm:hidden" aria-label="Close menu">
              <X className="w-6 h-6" />
            </button>
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-2 gap-8 lg:gap-12 h-full p-6 sm:p-8 md:p-12">
              ...
              <div className="space-y-6 sm:space-y-8">
                ...
                <nav className="space-y-2 sm:space-y-3" aria-label="Primary">
                  {navItems.map((item, index) => (
                    <motion.button
                      ...
                      className="block text-2xl sm:text-4xl md:text-5xl font-bold text-foreground hover:text-muted-foreground transition-colors duration-300 text-left"
                    >
                      {item.label}
                    </motion.button>
                  ))}
                ...
                <motion.button
                  ...
                  className="block text-2xl sm:text-4xl md:text-5xl font-bold text-muted-foreground hover:text-foreground transition-colors duration-300 text-left"
                >
                  Server Contents
                </motion.button>

                <div className="flex flex-wrap gap-3 pt-4 sm:pt-6 items-center">
                  <motion.a
                    ...
                    className="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-[hsl(235,86%,65%)] hover:bg-[hsl(235,86%,58%)] rounded-xl font-semibold text-white transition-all duration-300 text-sm sm:text-base"
                  >
                  ...
                  >
                    ...
                    <div className="flex flex-col leading-tight">
                      <span className={`font-semibold text-sm sm:text-base transition-colors duration-500 ${isLoading ? 'text-gray-400' : isOnline ? 'text-green-400' : 'text-red-400'}`}>
                        Server {statusText}
                      </span>
                      <span className="text-xs text-muted-foreground">{isLoading ? 'Checking players…' : `${playersText} Players`}</span>
                    </div>
                  </motion.div>
                </div>
              </div>
              ...
              >
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-lg sm:text-xl font-bold text-foreground">{adminPanel.owner}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-lg sm:text-xl font-bold text-foreground">{adminPanel.ceo}</p>
                  </div>

                  <div>
                    <p className="text-muted-foreground text-xs uppercase tracking-wider mb-2">Wandering Trader</p>
                    <p className="text-lg sm:text-xl font-bold text-foreground">{adminPanel.webDev}</p>
                  ...
                  <div>
                    ...
                    <p className="text-lg sm:text-xl font-bold text-foreground">{adminPanel.wither}</p>
                  </div>
                </div>
              </motion.div>
            </div>
